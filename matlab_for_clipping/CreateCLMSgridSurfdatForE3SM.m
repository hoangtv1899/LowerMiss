...
function fname_out = CreateCLMSgridSurfdatForE3SM(...
                    ...
                    out_netcdf_dir, clm_usrdat_name, scale_factor)
...
lonlen = lonlen * scale_factor;
latlen = latlen * scale_factor;
...
%             disp(data(ixmin:ixmax,iymin:iymax));
            netcdf.putVar(ncid_out,ivar-1,imresize(data(ixmin:ixmax,iymin:iymax),[lonlen latlen]));
        case 3
            netcdf.putVar(ncid_out,ivar-1,imresize(data(ixmin:ixmax,iymin:iymax,:),[lonlen latlen]));
        case 4
            netcdf.putVar(ncid_out,ivar-1,zeros(length(size(imresize(data(ixmin:ixmax,iymin:iymax,:,:),[lonlen latlen]))),1)',size(imresize(data(ixmin:ixmax,iymin:iymax,:,:),[lonlen latlen])), imresize(data(ixmin:ixmax,iymin:iymax,:,:),[lonlen latlen]));
        otherwise
            disp('error')
    end
end
